...
func (s *Store) UpdateProject(ctx context.Context, project models.Project) error {
	_, err := s.db.ExecContext(ctx, `
		UPDATE projects
...
func (s *Store) SaveSchemaRevision(ctx context.Context, projectID string, blueprint models.SchemaBlueprint, blueprintHash, generatedSQL string) (models.SchemaRevision, error) {
	...
	revision := models.SchemaRevision{
		ID:            uuid.NewString(),
		ProjectID:     projectID,
	...
	if err != nil {
		return models.SchemaRevision{}, err
	}
	_, err = s.db.ExecContext(ctx, `
...
func (s *Store) GetLatestSchemaRevision(ctx context.Context, projectID string) (models.SchemaRevision, error) {
	row := s.db.QueryRowContext(ctx, `
...
func (s *Store) ListSchemaRevisions(ctx context.Context, projectID string) ([]models.SchemaRevision, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, project_id, version_number, blueprint_json, blueprint_hash, generated_sql, created_at
		FROM schema_revisions
		WHERE project_id = ?
	...
	defer rows.Close()
	revisions := make([]models.SchemaRevision, 0)
	for rows.Next() {
...
func (s *Store) CreateApplyRun(ctx context.Context, run models.ApplyRun) error {
	_, err := s.db.ExecContext(ctx, `
...
func (s *Store) UpdateApplyRun(ctx context.Context, run models.ApplyRun) error {
	_, err := s.db.ExecContext(ctx, `
		UPDATE apply_runs
...
func scanRevision(scanner interface{ Scan(dest ...any) error }) (models.SchemaRevision, error) {
	var revision models.SchemaRevision
	var raw string
	...
	if err := json.Unmarshal([]byte(raw), &revision.Blueprint); err != nil {
		return models.SchemaRevision{}, fmt.Errorf("decode blueprint: %w", err)
	}
